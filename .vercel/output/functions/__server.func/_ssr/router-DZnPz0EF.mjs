...
import { a as getSql, n as dbSource, o as isServerlessRuntime, t as MISSING_DATABASE_URL_MESSAGE } from "./db-rIvx0piq.mjs";
import { n as auth } from "./server-D6u5cCP7.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/router-DZnPz0EF.js
...
var styles_default = "/assets/styles-CY1Ct2mz.css";
...
var Route$6 = createFileRoute("/api/health")({ server: { handlers: { GET: async () => {
	...
	if (!hasDatabaseUrl && serverless) {
		...
		result.fix = "Your auth is ready, but no Postgres was attached on publish. Republish and ensure the platform injects DATABASE_URL (Neon), or add your own Neon connection string as DATABASE_URL in the deploy env. You do NOT need a separate always-on server — Vercel + Neon is permanent.";
		return json(result, 503);
	}
	try {
		const sql = await getSql();
		result.db = (await sql`select 1::int as n`)[0]?.n === 1 ? "up" : "unexpected";
		result.tables = (await sql`
            select table_name as name from information_schema.tables
            where table_schema = 'public'
              and table_name in ('user', 'session', 'verification', 'account', 'profiles', 'couples')
            order by table_name`).map((t) => t.name);
		try {
			await sql`
