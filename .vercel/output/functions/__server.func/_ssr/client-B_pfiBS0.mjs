...
import { $t as capitalizeFirstLetter, en as toKebabCase, nn as createFetch, rn as isSafeUrlScheme } from "../_libs/@better-auth/core+[...].mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/client-B_pfiBS0.js
var client_B_pfiBS0_exports = /* @__PURE__ */ __exportAll({
	i: () => signOut,
	n: () => client_exports,
	r: () => signIn,
	t: () => authClient
...
function withTimeout(promise, ms) {
	return new Promise((resolve, reject) => {
		const t = window.setTimeout(() => reject(/* @__PURE__ */ new Error("Sign-in timed out — try again")), ms);
		promise.then((v) => {
...
async function signIn(providerId, opts = {}) {
	...
	if (Boolean(getBearerToken()) || !inLivePreview()) try {
		await withTimeout(authClient.signOut().then(() => void 0), 2500);
	} catch {}
	setBearerToken(null);
	if (inLivePreview()) {
	...
	let result;
	try {
		result = await withTimeout(authClient.signIn.oauth2({
			providerId,
			callbackURL,
			errorCallbackURL
		}), 2e4);
	...
	if (result.error) throw new Error(formatAuthError(result.error));
	...
	throw new Error("Could not start Google/X sign-in (no redirect URL). Open /api/health on this site — if the database is down, republish after the platform finishes provisioning.");
}
...
//#endregion
export { signOut as i, client_B_pfiBS0_exports as n, signIn as r, authClient as t };
