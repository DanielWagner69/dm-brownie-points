//#region node_modules/.nitro/vite/services/ssr/assets/db-BWLI4Tr_.js
...
function loadMigrationFiles() {
	return [{
		name: "0001_auth.sql",
		text: _0001_auth_default
	}, {
		name: "0002_paws.sql",
		text: _0002_paws_default
	}];
}
async function applyMigrationsWithPool(pool) {
	const appliedNow = [];
	...
	try {
		...
		for (const { name, text } of loadMigrationFiles()) {
			...
			if (!text || !String(text).trim()) throw new Error(`Migration ${name} is empty — SQL was not bundled`);
			try {
...
function createNeonSql() {
	globalRef.__pgSqlPromise__ ??= (async () => {
		...
		const pass = (globalRef.__neonMigrateChain__ ?? Promise.resolve()).catch(() => void 0).then(() => applyMigrationsWithPool(pool));
		globalRef.__neonMigrateChain__ = pass.then(() => void 0);
		await pass;
