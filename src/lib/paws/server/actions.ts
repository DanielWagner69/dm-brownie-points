...
export const logAction = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    // "both" still attributes points to one person; default to partner (shared moment often credited to the other).
    const applies_to =
      data.direction === "self" ? userId : partner;
  ...
  .handler(async ({ context, data }) => {
    ...
    const applies_to =
      direction === "self"
        ? a.logged_by
        : direction === "partner"
          ? partnerIdOf(c, a.logged_by) ?? a.applies_to
          : a.applies_to;
