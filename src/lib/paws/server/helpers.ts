/**
 * Internal helpers for paws server modules.
 * Not exported as createServerFn – imported by other server modules.
 */
...
export async function ensureProfile(userId: string, fallbackName: string): Promise<Profile> {
...
export async function getActiveCouple(userId: string): Promise<{
  id: string;
  invite_code: string;
  user_a: string;
  user_b: string | null;
...
export function buildTreatTips(
  balance: number,
...
export async function seedDefaults(coupleId: string, userId: string) {
  const sql = await getSql();
...
export async function syncDefaultActions(coupleId: string, userId: string) {
  const sql = await getSql();
...
export async function syncCategories(coupleId: string) {
  const sql = await getSql();
...
export async function computeBalanceLive(userId: string, coupleId: string): Promise<Balance> {
  const sql = await getSql();
...
export async function updateStreak(userId: string, coupleId: string) {
  const sql = await getSql();
...
export async function evaluateBadges(userId: string, coupleId: string) {
  const sql = await getSql();
...
export async function buildExportCsv(coupleId: string): Promise<string> {
  const sql = await getSql();
